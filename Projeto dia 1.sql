...
-- FAIXA ETÁRIA DE TODOS OS CLIENTES
WITH TABELA_IDADE (NOME, IDADE, FAIXA_ETARIA)
AS (
SELECT
	C.FirstName AS NOME
	,2008 -YEAR(C.BirthDate) AS IDADE
	,CASE
	 WHEN 2008 -YEAR(C.BirthDate) BETWEEN 18 AND 30 THEN '18-30'
		WHEN 2008 -YEAR(C.BirthDate) BETWEEN 31 AND 59 THEN '31-59'
		WHEN 2008 -YEAR(C.BirthDate) >= 60 THEN '60+'
	END AS FAIXA_ETARIA
FROM DimCustomer C
)
